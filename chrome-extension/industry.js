...
class IndustryManager {
    ...
    chooseIndustry(key, world) {
        if (this.industries[key]) return { ok: false, error: '已擁有此產業' };
        if (Object.keys(this.industries).length >= this.maxIndustries) return { ok: false, error: '產業上限已滿' };
        if (!INDUSTRIES[key]) return { ok: false, error: '未知產業' };
        ...
        if (world) {
            world.logMessage('industry', `${def.icon} 開啟了${def.name}！`);
            if (world.dailyNews) world.dailyNews.collectEvent('industry', `城鎮開啟了新產業：${def.name}！`, 7);
        }
        return { ok: true };
    }
