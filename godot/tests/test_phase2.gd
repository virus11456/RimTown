...
func run() -> void:
	...
	for lang in ["auto","zh","en"]:
		result = await api.set_settings(20,lang)
		check(result.ok and result.data.dialogue_lang == lang,"dialogue preference "+lang)
	result = await api.set_settings(20,"invalid")
	check(not result.ok,"invalid dialogue preference rejected")
	result = await api.chat("fixture prompt","chat",400,0.7,"en")
	check(result.ok and result.data.lang == "en","English chat contract")
	result = await api.chat("fixture prompt","chat",400,0.7,"invalid")
	check(not result.ok,"invalid chat language rejected")
	...
	result = await api.register("fixture-user","fixture-password","fixture@example.invalid","fixture-invite")
