...
func _initialize() -> void:
	for theme in ["frontier","harbor"]:
		var fixture: Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://tests/social/"+theme+"-simulation.json"))
	...
	var report:={"checks":checks,"failures":failures,"scope":"two towns x 2880 ticks: Phase 4a + NPC socializing and offline conversation; gossip/news and other systems excluded"}
	FileAccess.open("res://docs/NPC_SOCIAL_TESTS.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "))
