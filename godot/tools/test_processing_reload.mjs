...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/processing/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
...
const report={checks,passed:true,scope:'Original JS loads building/trade/research/industry/farm/processing state and stock, memories, relationships and currentThought. Original reload drops _rivalGossiped; Godot resume retains it. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/PROCESSING_RELOAD_TESTS.json'),json(report));console.log(json(report));
