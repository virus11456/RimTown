...
for(const theme of ['frontier','harbor']){
 ...
 if(stargazing){input.clock.hour=21;input.clock.minute=0;for(const a of Object.values(input.agents).filter(a=>!a.isPlayer).slice(0,4)){a.personality.traits=["night_owl"];a.needs.rest=100;}}
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;ctx.enableRomance=romance;ctx.enableFeuds=feuds;ctx.enableFactions=factions;ctx.enableThoughts=thoughts;ctx.enableInnerVoice=innerVoice;ctx.enableStargazing=stargazing;
 vm.runInContext(`const savedMessages=JSON.parse(JSON.stringify(input.messageLog||[]));var w=new World();w.loadSave(input);w.reputationSystem.getModifier=()=>0;Object.defineProperty(w.weather,'moodModifier',{get:()=>0});w.messageLog=savedMessages;const originalLog=w.logMessage.bind(w);w.logMessage=(type,...args)=>{if(['social','conversation',...(enableStargazing?['discovery']:[]),...(enableGossip?['gossip']:[]),...(enableRomance?['relationship']:[]),...(enableFeuds?['event']:[]),...(enableFactions?['faction']:[])].includes(type))originalLog(type,...args);};w.dailyNews.collectEvent=()=>{};w.queueDramaScene=()=>{};w.npcEvents.handleCheatingDiscovery=()=>{};if(!enableGossip)w.gossipNetwork.spreadGossip=()=>{};
 for(const name of ['_doNightMischief','_doMourning',...(!enableInnerVoice?['_generateThought']:[]),'_recordTrace','_perceiveSurroundings'])Agent.prototype[name]=function(){};`,ctx);
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${stargazing?'-stargazing':innerVoice?'-inner-voice':thoughts?'-thoughts':factions?'-factions':feuds?'-feuds':romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
