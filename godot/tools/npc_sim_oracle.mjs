...
const gossip=process.argv.includes('--gossip');
for(const theme of ['frontier','harbor']){
 ...
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;
 vm.runInContext(`const savedMessages=JSON.parse(JSON.stringify(input.messageLog||[]));var w=new World();w.loadSave(input);w.reputationSystem.getModifier=()=>0;Object.defineProperty(w.weather,'moodModifier',{get:()=>0});w.messageLog=savedMessages;const originalLog=w.logMessage.bind(w);w.logMessage=(type,...args)=>{if(['social','conversation',...(enableGossip?['gossip']:[])].includes(type))originalLog(type,...args);};w.dailyNews.collectEvent=()=>{};if(!enableGossip)w.gossipNetwork.spreadGossip=()=>{};
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
