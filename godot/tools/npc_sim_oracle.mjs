...
for(const theme of ['frontier','harbor']){
 ...
 if(mourning){for(const a of Object.values(input.agents).filter(a=>!a.isPlayer).slice(0,4)){a._mourningTargets=[{name:"故友",isFamily:false},{name:"家人",isFamily:true}];a._annualMourning=[{name:"先人",lastVisitYear:0}];}}
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;ctx.enableRomance=romance;ctx.enableFeuds=feuds;ctx.enableFactions=factions;ctx.enableThoughts=thoughts;ctx.enableInnerVoice=innerVoice;ctx.enableStargazing=stargazing;ctx.enableMischief=mischief;ctx.enableMourning=mourning;ctx.enableTrace=trace;ctx.enablePerception=perception;
 vm.runInContext(`const savedMessages=JSON.parse(JSON.stringify(input.messageLog||[]));var w=new World();w.loadSave(input);w.reputationSystem.getModifier=()=>0;Object.defineProperty(w.weather,'moodModifier',{get:()=>0});w.messageLog=savedMessages;const originalLog=w.logMessage.bind(w);w.logMessage=(type,...args)=>{if(['social','conversation',...(enableMourning?['mourning']:[]),...(enableMischief?['mischief']:[]),...(enableStargazing?['discovery']:[]),...(enableGossip?['gossip']:[]),...(enableRomance?['relationship']:[]),...(enableFeuds?['event']:[]),...(enableFactions?['faction']:[])].includes(type))originalLog(type,...args);};w.dailyNews.collectEvent=()=>{};w.queueDramaScene=()=>{};w.npcEvents.handleCheatingDiscovery=()=>{};if(!enableGossip)w.gossipNetwork.spreadGossip=()=>{};
 for(const name of [...(!enableMischief?['_doNightMischief']:[]),...(!enableMourning?['_doMourning']:[]),...(!enableInnerVoice?['_generateThought']:[]),...(!enableTrace?['_recordTrace']:[]),...(!enablePerception?['_perceiveSurroundings']:[])])Agent.prototype[name]=function(){};`,ctx);
 ...
 for(let tick=1;tick<=2880;tick++){
  ...
  if(romance)for(const [id,a] of Object.entries(agents)){
   ...
   if(perception)a.observation=vm.runInContext(`({day:w.agents[${JSON.stringify(id)}]._obsDay||"",count:w.agents[${JSON.stringify(id)}]._obsCount||0})`,ctx);
   if(trace){a.todayTrace=structuredClone(save.agents[id].todayTrace||[]);a._traceDay=save.agents[id]._traceDay||"";}
   if(mourning){a._mourningTargets=structuredClone(save.agents[id]._mourningTargets);a._annualMourning=structuredClone(save.agents[id]._annualMourning);}
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${perception?'-perception':trace?'-trace':mourning?'-mourning':mischief?'-mischief':stargazing?'-stargazing':innerVoice?'-inner-voice':thoughts?'-thoughts':factions?'-factions':feuds?'-feuds':romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
