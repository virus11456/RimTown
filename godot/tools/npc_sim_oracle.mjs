...
for(const theme of ['frontier','harbor']){
 ...
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;ctx.enableRomance=romance;ctx.enableFeuds=feuds;ctx.enableFactions=factions;ctx.enableThoughts=thoughts;ctx.enableInnerVoice=innerVoice;ctx.enableStargazing=stargazing;ctx.enableMischief=mischief;ctx.enableMourning=mourning;
 ...
 for(const name of [...(!enableMischief?['_doNightMischief']:[]),...(!enableMourning?['_doMourning']:[]),...(!enableInnerVoice?['_generateThought']:[]),'_recordTrace','_perceiveSurroundings'])Agent.prototype[name]=function(){};`,ctx);
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${mourning?'-mourning':mischief?'-mischief':stargazing?'-stargazing':innerVoice?'-inner-voice':thoughts?'-thoughts':factions?'-factions':feuds?'-feuds':romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
