...
for(const theme of ['frontier','harbor']){
 ...
 for(const name of [...(!enableMischief?['_doNightMischief']:[]),...(!enableMourning?['_doMourning']:[]),...(!enableInnerVoice?['_generateThought']:[]),...(!enableTrace?['_recordTrace']:[]),'_perceiveSurroundings'])Agent.prototype[name]=function(){};`,ctx);
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${trace?'-trace':mourning?'-mourning':mischief?'-mischief':stargazing?'-stargazing':innerVoice?'-inner-voice':thoughts?'-thoughts':factions?'-factions':feuds?'-feuds':romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
