...
const feuds=process.argv.includes('--feuds');const romance=feuds||process.argv.includes('--romance');const gossip=romance||process.argv.includes('--gossip');
for(const theme of ['frontier','harbor']){
 ...
 for(let tick=1;tick<=2880;tick++){
  vm.runInContext("w.tickCount++;if(w.clock.tick().includes('new_day')){if(enableRomance)w._processRelationships();if(enableFeuds)w._processFeuds();}for(const a of Object.values(w.agents))a.update(w);",ctx);
