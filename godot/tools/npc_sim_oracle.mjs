...
const factions=process.argv.includes('--factions');const feuds=factions||process.argv.includes('--feuds');const romance=feuds||process.argv.includes('--romance');const gossip=romance||process.argv.includes('--gossip');
for(const theme of ['frontier','harbor']){
 ...
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;ctx.enableRomance=romance;ctx.enableFeuds=feuds;ctx.enableFactions=factions;
 ...
 for(let tick=1;tick<=2880;tick++){
  vm.runInContext("w.tickCount++;if(w.clock.tick().includes('new_day')){if(enableRomance)w._processRelationships();if(enableFeuds)w._processFeuds();if(enableFactions)w.factions.dailyUpdate(w);}for(const a of Object.values(w.agents))a.update(w);",ctx);
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${factions?'-factions':feuds?'-feuds':romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
