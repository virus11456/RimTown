...
const romance=process.argv.includes('--romance');const gossip=romance||process.argv.includes('--gossip');
for(const theme of ['frontier','harbor']){
 ...
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;ctx.enableRomance=romance;
 vm.runInContext(`const savedMessages=JSON.parse(JSON.stringify(input.messageLog||[]));var w=new World();w.loadSave(input);w.reputationSystem.getModifier=()=>0;Object.defineProperty(w.weather,'moodModifier',{get:()=>0});w.messageLog=savedMessages;const originalLog=w.logMessage.bind(w);w.logMessage=(type,...args)=>{if(['social','conversation',...(enableGossip?['gossip']:[]),...(enableRomance?['relationship']:[])].includes(type))originalLog(type,...args);};w.dailyNews.collectEvent=()=>{};w.queueDramaScene=()=>{};w.npcEvents.handleCheatingDiscovery=()=>{};if(!enableGossip)w.gossipNetwork.spreadGossip=()=>{};
 ...
 for(let tick=1;tick<=2880;tick++){
  vm.runInContext("w.tickCount++;if(w.clock.tick().includes('new_day')&&enableRomance)w._processRelationships();for(const a of Object.values(w.agents))a.update(w);",ctx);
  ...
  checkpoints.push({...gossip?{gossip:structuredClone(save.gossip),townFeed:structuredClone(save.townFeed)}:{},tick,rng:c.getRandomState(),clock:save.clock,agents,conversationCount:save.npcConversationLog.length,conversationHash:sha(save.npcConversationLog.flatMap(c=>c.dialogue.map(d=>d.speaker+':'+d.text)).join('\n')),conversationTail:save.npcConversationLog.slice(-5),logCount:save.messageLog.length,logTail:save.messageLog.slice(-5)});
 }
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
