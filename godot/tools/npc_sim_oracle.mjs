...
const thoughts=process.argv.includes('--thoughts');const factions=thoughts||process.argv.includes('--factions');const feuds=factions||process.argv.includes('--feuds');const romance=feuds||process.argv.includes('--romance');const gossip=romance||process.argv.includes('--gossip');
for(const theme of ['frontier','harbor']){
 ...
 ctx.input=structuredClone(input);ctx.enableGossip=gossip;ctx.enableRomance=romance;ctx.enableFeuds=feuds;ctx.enableFactions=factions;ctx.enableThoughts=thoughts;
 ...
 fs.writeFileSync(path.join(root,`godot/tests/social/${theme}${thoughts?'-thoughts':factions?'-factions':feuds?'-feuds':romance?'-romance':gossip?'-gossip':''}-simulation.json`),json({input,checkpoints}));console.log(theme,'7 social simulation checkpoints');
}
