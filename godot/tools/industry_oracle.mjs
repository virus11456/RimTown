...
const cases=[],inputs=[],inputIndex=new Map();
for(const scenario of ['baseline','empty_workers','town_promotion','unavailable_workers'])for(let mask=1;mask<16;mask++)for(const level of [1,2,3,4,5]){
 ...
 const out=JSON.parse(vm.runInContext('JSON.stringify(w.serialize())',c.ctx));const serialized=JSON.stringify(input);if(!inputIndex.has(serialized)){inputIndex.set(serialized,inputs.length);inputs.push(input)}cases.push({input:inputIndex.get(serialized),keys:JSON.parse(vm.runInContext('JSON.stringify(keys)',c.ctx)),level,industry:out.industry,stock:out.stockpile,logs:out.messageLog,news:out.dailyNews});
}
fs.mkdirSync(path.join(root,'godot/tests/industry'),{recursive:true});fs.writeFileSync(path.join(root,'godot/tests/industry/oracle.json'),json({inputs,cases}));console.log(cases.length,'industry cases');
