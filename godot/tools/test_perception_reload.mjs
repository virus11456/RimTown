...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/perception/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
assert.equal(expected.agents.chen_wei.memory.filter(e=>e.category==="observation").length,6);
...
const report={checks,passed:true,scope:'Original JS loads observations, memory, relationships and currentThought. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/PERCEPTION_RELOAD_TESTS.json'),json(report));console.log(json(report));
