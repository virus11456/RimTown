...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/economy/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
...
for(const key of ['clock','factions','events','gossip','stockpile','playerActions','workPolicy']){assert.deepEqual(actual[key],expected[key]);checks++;}
...
const report={checks,passed:true,scope:'Original JS loads daily production stock and ledger, memories, relationships and currentThought. Original reload drops _rivalGossiped; Godot resume retains it. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/ECONOMY_RELOAD_TESTS.json'),json(report));console.log(json(report));
