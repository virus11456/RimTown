...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/player_gift/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
...
for(const key of ['clock','factions','events','gossip','stockpile','playerActions']){assert.deepEqual(actual[key],expected[key]);checks++;}
...
const report={checks,passed:true,scope:'Original JS loads gift history and stock, memories, relationships and currentThought. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/PLAYER_GIFT_RELOAD_TESTS.json'),json(report));console.log(json(report));
