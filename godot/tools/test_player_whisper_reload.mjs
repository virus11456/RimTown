...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/player_whisper/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
assert.equal(expected.agents.player.chatHistory.length,2);
assert.equal(expected.agents.player.chatHistory.at(-1)._godotOffline,true);
...
const report={checks,passed:true,scope:'Original JS loads whisper history, provenance, memories, relationships and currentThought. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/PLAYER_WHISPER_RELOAD_TESTS.json'),json(report));console.log(json(report));
