...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/quests/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
...
for(const key of ['clock','factions','events','gossip','stockpile','playerActions','workPolicy','buildings','trade','research','industry','farm','processing','decorations','combosFound','heartEventsFired','questSystem','npcQuests','lifeGoals','multiEnding']){assert.deepEqual(actual[key],expected[key]);checks++;}
...
const report={checks,passed:true,scope:'Original JS loads quest/NPC/life/ending states plus building/trade/research/industry/farm/processing state and stock, memories, relationships and currentThought. Original reload drops _rivalGossiped; Godot resume retains it. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/QUEST_RELOAD_TESTS.json'),json(report));console.log(json(report));
