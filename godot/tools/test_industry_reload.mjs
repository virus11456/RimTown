...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/industry/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
...
for(const key of ['clock','factions','events','gossip','stockpile','playerActions','workPolicy','buildings','trade','research','industry']){assert.deepEqual(actual[key],expected[key]);checks++;}
...
const report={checks,passed:true,scope:'Original JS loads building/trade/research/industry state and stock, memories, relationships and currentThought. Original reload drops _rivalGossiped; Godot resume retains it. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/INDUSTRY_RELOAD_TESTS.json'),json(report));console.log(json(report));
