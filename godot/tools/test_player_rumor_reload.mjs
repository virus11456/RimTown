...
const {ctx}=context();const expected=JSON.parse(fs.readFileSync(path.join(root,'godot/tests/player_rumor/compatibility-save.json.tmp')));ctx.input=structuredClone(expected);
...
const report={checks,passed:true,scope:'Original JS loads rumor history and gossip, memories, relationships and currentThought. No production API.'};
fs.writeFileSync(path.join(root,'godot/docs/PLAYER_RUMOR_RELOAD_TESTS.json'),json(report));console.log(json(report));
