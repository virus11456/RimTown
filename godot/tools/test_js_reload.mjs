...
assert.deepEqual(JSON.parse(JSON.stringify(output.clock)),ctx.input.clock);
assert.equal(Object.keys(output.agents).length,Object.keys(ctx.input.agents).length);
for(const[id,a]of Object.entries(ctx.input.agents))for(const key of ['currentLocation','activity','needs','skills'])assert.deepEqual(JSON.parse(JSON.stringify(output.agents[id][key])),a[key]);
