...
在 Phase 0–3 觀賞版上加入時鐘、需求、作息、技能成長、A* 尋路、進出建築與卡住復原。兩鎮範例可直接開啟；預設暫停，WASD／方向鍵可直接移動旅人，按「▶ 開始」則推進村民作息。這是分階段試玩版，現已加入 NPC 本地規則對話、八卦傳播、每日戀愛／分離、仇怨、派系事件、每日想法更新、日常心聲、觀星互動、夜間惡作劇、居民弔念與今日足跡；尚未包含建設、自主 AI 行程、經濟、婚禮演出及任務模擬。
...
## Phase 4b 第八批：居民日常心聲
居民資料與「目前想法」現在顯示此刻心聲，依原版心情、需求、夜間活動、夜貓子特質、跨鎮親友、人際關係與技能選句。存檔中的庫存、工程、商人和新聞也能提供候選句；這不代表經濟或新聞模擬已移植。

按「▶ 開始」推進後，在「居民」選人查閱。設定中的「居民日常心聲」可停用日常抽選；技能升級的既有提示仍可能更新心聲。心聲是 currentThought 文字，不新增持續 thoughts 或直接改變心情。新匯入預設開啟，試玩進度保留開關與心聲；旅人不套用 NPC 抽選。
...
## Phase 4b 第十批：夜間惡作劇
NPC 處於夜間惡作劇活動時，依原版機率觸發六種事件。成功後更新心情修正、心聲、記憶與話題；被未睡的 NPC 撞見時，目擊者對當事人好感減 5，留下目擊記憶，當事人心情修正再減 5。原版目擊者從全鎮未睡且非玩家的角色選取，不限制同地點。

「居民 → 選人 → 近期記憶」可查閱惡作劇／目擊；「故事」保留事件。設定「夜間惡作劇」可停用事件效果，既有活動選擇不變。新匯入預設開啟，試玩進度保存開關與效果。沒有新增偷竊物資扣除、建物染色或追逐動畫：來源列出的 target／mood_others／severity 並未被此方法使用，依原版保留实际作用範圍。

驗證：336 組、每組 32 次原版呼叫，共 2,018 項；含全部六種事件、無目擊者、睡眠／玩家排除、多目擊者、遠處目擊者、好感下限與亂數。兩鎮各 30 天、四名夜貓子兼八卦特質居民測試 3,796 項；UI／續存 21 項；原版 JS 讀回 89 項。既有觀星、社交 UI、旅人與畫面回歸通過。完整 World.tick、弔念與 AI 演出仍待後續。

重建：`node tools/mischief_oracle.mjs`、`node tools/npc_sim_oracle.mjs --mischief`。Godot 測試：`tests/test_mischief.gd`、`tests/test_mischief_ui.gd`、`tests/test_npc_sim.gd -- --mischief`；JS 讀回：`node tools/test_mischief_reload.mjs`。


## Phase 4b 第十一批：近期弔念與年度追思
居民處於弔念活動時，按原版 15% 機率處理一筆資料：先依序處理近期名單，再查找年度尚未追思的親人。近期弔念增加 0.5 社交需求與 0.3 心情修正；親人按姓名去重加入年度名單。年度追思增加 1 社交需求、心情修正減 2，同年度不再重複。心情仍在原有下一次計算時反映。

「居民 → 選人 → 近期記憶」查看弔念紀錄，「目前想法」查看思念心聲。「設定 → 居民弔念」可停用效果，活動抽選仍沿用原有作息；試玩進度保留模式、待辦名單與上次追思年份。

目前處理已載入的 _mourningTargets／_annualMourning，未接入死亡事件產生新名單的完整系統。沒有新增逝者、死亡判定或墓園動作演出。心聲可能被後續日常抽選替換，記憶與故事紀錄仍會保留。

驗證：192 組跨兩年原版情境、1,536 項；兩鎮各 30 天加入近期與年度名單的限定整合 4,314 項；UI／續存 20 項，原版 JS 讀回 131 項。既有夜間活動／社交 UI、旅人與畫面回歸通過。完整 World.tick 仍未驗收。

重建：`node tools/mourning_oracle.mjs`、`node tools/npc_sim_oracle.mjs --mourning`。Godot 測試：`tests/test_mourning.gd`、`tests/test_mourning_ui.gd`、`tests/test_npc_sim.gd -- --mourning`；JS 讀回：`node tools/test_mourning_reload.mjs`。


## Phase 4b 第十二批：居民今日足跡
在「居民 → 選人 → 今日足跡」依時間查看活動與地點名稱。活動或地點改變才新增，當天最多 160 筆；首次更新到新日期時重置。昨天的資料在新一天尚未產生紀錄時不顯示為今天。此功能依原版只記錄 NPC，不記錄玩家鍵盤移動。

已載入的 dailyPlan.blocks 可提供目標及按時間分配的步驟；睡覺與進食優先。這只是原版足跡文字選取，沒有新增 AI 行程生成或讓角色依該文字執行新任務。「設定 → 居民足跡」可暫停記錄，重開不補寫空白時段。存檔保留日期、足跡與去重狀態。
...
## 玩家互動第一批：近身安慰
走近居民後按 F，或由居民資料按「與他互動」。沿用原版 2.5 格距離，超出距離時提示先走近；按鈕執行時再驗證，避免隔空操作。F 避開既有 Q／E 相機鍵，文字輸入／檔案視窗不觸發。

「安慰他」接入原版 _applyChatIntent 的 comfort 確定效果：社交 +15（上限 100）、心情修正 +4、NPC 對玩家好感 +2、「愉快的聊天」想法與 playerActions 記錄。重複安慰刷新想法日期而非堆疊同類想法。自由輸入 AI 對話、完整 playerSendMessage 周邊效果、送禮與其他意圖仍待接入；目前結果是規則回饋，不是 AI 回覆。原版近身距離未檢查視線，本輪同樣不新增穿牆視線判定。

驗證：24 組原版意圖情境 72 項；UI／距離／F 輸入守衛／續存 20 項；旅人、實際 Control 點擊、畫面與既有玩法回歸通過。測試顯示角色位置直接準備，不代表自動走路 E2E。


## 玩家互動第二批：自由交談與記憶上下文
「居民 → 選人 → 與他互動 → 自由交談」可輸入訊息；F 近身入口也可進入。自由交談依原版不限制距離，安慰仍維持近身驗證。傳送使用既有 `/api/chat`、chat 通道、繁中、600 tokens、temperature 0.9；登入沿用設定中的帳號，未登入走服務的訪客額度。沒有雲端存檔寫入。

提示包含角色背景／特質／關係、相關記憶 5 則、反思 2 則及最近對話最多 10 則。為適應後端 6000 UTF-16 字元截斷，以 5800 為預算，過長時縮減舊對話／記憶，保留本次訊息及格式。這是本版精簡上下文，不宣稱與原版整段提示逐字相同；經濟／任務提示与未移植心動事件、重排行程、任務 onChat 回呼不在本輪。

成功回覆套用原版核心關係／記憶／聊天紀錄與 recentChatTick。訊息最多 1200 字，單一請求在途；草稿不受世界 tick 刷新影響。失敗／空回覆／非法效果不寫入遊戲效果或聊天紀錄，保留草稿重試；換鎮／讀檔丟棄舊回覆。為防異常模型資料，數字效果限制為好感 -3..5、戀慕 0..3；原版未強制此範圍。沒有把離線固定句冒充 AI 回覆。

驗證：32 組原版解析與核心效果對照 165 項、mock 傳輸 UI 24 項、原版 JS 讀回 172 項。本機 HTTP 契約 55 項／16 requests 通過。近身／旅人／Control 點擊／足跡／見聞回歸通過。正式服務成功率、帳號額度與回覆品質尚未實測；截圖是 mock 回覆。聊天 timeout 35 秒以涵蓋後端 26 秒共用 deadline，其他 API 維持原設定。

重建：`node tools/player_chat_oracle.mjs`；Godot：`tests/test_player_chat.gd`、`tests/test_player_chat_ui.gd`；JS：`node tools/test_player_chat_reload.mjs`。
