...
extends RefCounted
static func allowed(save: Dictionary,site: Vector2i) -> bool:
	var layout:=TownLayout.new();layout.rebuild(save)
	if site.x<2 or site.y<2 or site.x>76 or site.y>56: return false
	if not layout._clear(site.x,site.y,2,2): return false
	var zones:=reserved(layout,save)
	if layout._overlap(site.x,site.y,2,2,zones): return false
	for y in range(site.y,site.y+2):
		for x in range(site.x,site.x+2):
			if layout.grid[y][x] not in [0,1,2]: return false
	return true
static func candidates(save: Dictionary) -> Array[Vector2i]:
	...
	for y in range(2,57,2):
		for x in range(2,77,2):
			if not layout._clear(x,y,2,2) or layout._overlap(x,y,2,2,zones): continue
			var clear:=true
